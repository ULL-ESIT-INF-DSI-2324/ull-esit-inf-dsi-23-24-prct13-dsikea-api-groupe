...
transactionRouter.post('/transaction', async (req, res) => {
    try {
        ...
        if (furnitures.length <= 0)
            return res.status(404).send("Furnitures has been not provide");
        if (type == "Sell") { // Proveedor vende
            if (customer)
                return res.status(404).send("You need to introduce a provider, not a customer");
            ...
            if (provider)
                return res.status(404).send("You need to introduce a customer, not a provider");
            const transaction = new Transactions(req.body);
            transaction.save().then((tsn) => {
                res.send(tsn);
            }).catch((err) => {
